...
class AirChinaConnectorClient:
    ...
    async def _search_ow(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        # NOTE: do NOT call auto_block_if_proxied() — Air China's Next.js SPA
        # requires images, fonts, websockets and its JD Cloud captcha/fireeye
        # behavioral tracking to function. Blocking them causes the SPA to fail
        # silently (4000+ net::ERR_FAILED, search never navigates).
        # Instead, only block Chrome background data hogs.
        try:
            await page.route("**/*optimizationguide-pa.googleapis.com*", lambda r: r.abort())
            await page.route("**/*safebrowsing.googleapis.com*", lambda r: r.abort())
            await page.route("**/*edgedl.me.gvt1.com*", lambda r: r.abort())
            await page.route("**/*update.googleapis.com*", lambda r: r.abort())
        except Exception:
            pass
        ...
        _all_network_urls: list = []  # diagnostic: capture ALL network traffic

        async def _on_response(response):
            url = response.url.lower()
            status = response.status
            # ── Diagnostic: log ALL non-asset responses ──
            if not any(url.endswith(ext) for ext in (".png", ".jpg", ".gif", ".svg", ".ico", ".woff", ".woff2", ".ttf", ".css")):
                _all_network_urls.append(f"[{status}] {response.url[:120]}")
                if status >= 400:
                    logger.info("AirChina NET: [%d] %s", status, response.url[:150])
            if status not in (200, 201):
                return

            # ── Priority 1: exact flight/list endpoint (most reliable) ──
            is_flight_api = "/gateway/api/flight/list" in url or "/flight/list" in url
            # ── Priority 2: other known API patterns ──
            is_other_api = (not is_flight_api and 
                            any(k in url for k in ["/availability", "/offer", "/fare",
                                                    "/lowprice", "/schedule"]))
            if not is_flight_api and not is_other_api:
                return
            try:
                ...
                logger.info("AirChina API-HIT: [%d] %s ct=%s len=%d body=%.300s",
                            status, url[:100], ct, len(body), body[:300])
                if len(body) < 50:
                    return
                ...
                if "Data" in data and isinstance(data.get("Data"), dict):
                    if "Journeys" in data["Data"]:
                        ...
                        return
                # For the specific flight/list endpoint, capture even if structure is unexpected
                if is_flight_api:
                    search_data.clear()
                    search_data.update(data)
                    api_event.set()
                    logger.info("AirChina: captured flight/list (unexpected structure) keys=%s", list(data.keys())[:10])
                    return
                # Fallback: check for common flight data keys (NOT "result" — too generic)
                ...
                logger.warning("AirChina: response intercept error for %s: %s", url[:80], e)

        # ── Diagnostic: capture JS console errors ──
        _console_errors: list = []
        def _on_console(msg):
            if msg.type in ("error", "warning"):
                _console_errors.append(f"[{msg.type}] {msg.text[:200]}")
                if msg.type == "error":
                    logger.info("AirChina CONSOLE: %s", msg.text[:200])
        ...
        try:
            ...
            # ── Fallback: if search click didn't navigate, try direct URL ──
            current_url = page.url
            if "flight" not in current_url.lower() and not api_event.is_set():
                direct_url = f"https://www.airchina.com.cn/flight/oneway/{req.origin.lower()}-{req.destination.lower()}/{date_str}"
                logger.info("AirChina: search didn't navigate (still at %s), trying direct URL: %s", current_url[:60], direct_url)
                try:
                    await page.goto(direct_url, wait_until="domcontentloaded", timeout=20000)
                    logger.info("AirChina: direct URL loaded, waiting for SPA hydration...")
                    # Wait for the SPA to hydrate and make API calls
                    for _wait in range(12):  # up to 24 seconds
                        await asyncio.sleep(2.0)
                        if api_event.is_set():
                            logger.info("AirChina: API data captured after direct navigation")
                            break
                        # Check if flights appeared in DOM
                        has_data = await page.evaluate(r"""() => {
                            const text = document.body.innerText || '';
                            return /CA\d{3,4}/.test(text) && /[￥¥]\d{3,}/.test(text);
                        }""")
                        if has_data:
                            logger.info("AirChina: flight data visible after direct navigation (%ds)", (_wait + 1) * 2)
                            break
                except Exception as e:
                    logger.warning("AirChina: direct URL navigation failed: %s", e)
            ...
            if not offers:
                offers = await self._scrape_dom(page, req)
            # ── Late arrival check: flight/list may arrive after DOM scrape ──
            if not offers and search_data:
                logger.info("AirChina: re-checking search_data after DOM scrape (keys=%s)", list(search_data.keys())[:8])
                offers = self._parse_api_response(search_data, req)
            if not offers:
                # One more wait for late API responses
                await asyncio.sleep(3.0)
                if search_data and "Data" in search_data:
                    logger.info("AirChina: late API data arrived, re-parsing")
                    offers = self._parse_api_response(search_data, req)
            ...
            if not offers:
                try:
                    ...
                    # ── Diagnostic dump ──
                    logger.info("AirChina DIAG: %d network responses captured", len(_all_network_urls))
                    for nu in _all_network_urls[-50:]:
                        logger.info("AirChina NET-URL: %s", nu)
                    if _console_errors:
                        logger.info("AirChina DIAG: %d console errors", len(_console_errors))
                        for ce in _console_errors[:20]:
                            logger.info("AirChina JS-ERR: %s", ce)
                    # Page HTML snippet
                    try:
                        html_snippet = await page.evaluate("() => document.documentElement.outerHTML.substring(0, 3000)")
                        logger.info("AirChina HTML: %s", html_snippet)
                    except Exception:
                        pass
                    try:
                        body_text = await page.evaluate("() => (document.body && document.body.innerText || '').substring(0, 1000)")
                        logger.info("AirChina BODY-TEXT: %s", body_text)
                    except Exception:
                        pass
                except Exception as e:
