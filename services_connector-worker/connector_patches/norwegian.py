...
1. ONCE per ~25 min: Playwright opens booking.norwegian.com to farm Incapsula
   cookies (reese84, visid_incap, etc.).
...
class NorwegianConnectorClient:
    """Norwegian scraper — Playwright form fill + response interception."""
    ...
    async def _search_ow(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        With Playwright interception each search takes ~40s, so we limit city
        expansion to primary airports only (first in list) to keep total time
        reasonable. Norwegian's search API already shows all options within
        each airport pair.
        """
        origins = get_city_airports(req.origin)
        destinations = get_city_airports(req.destination)

        # For interception mode, only use the first (primary) airport per city
        # to avoid 10+ minute searches (each sub-search takes ~40s)
        if len(origins) > 2:
            origins = origins[:1]
