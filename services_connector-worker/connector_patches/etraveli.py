...
PATCH: Replace httpx with curl_cffi to bypass Akamai TLS fingerprinting.
"""
...
class EtraveliConnectorClient:
    ...
    async def _do_search(
        ...
        # Use curl_cffi with Chrome TLS fingerprint to bypass Akamai
        ...
        if resp.status_code != 200:
            logger.warning("ETRAVELI(%s) HTTP %d: %s", self.brand, resp.status_code, resp.text[:200])
            return None
