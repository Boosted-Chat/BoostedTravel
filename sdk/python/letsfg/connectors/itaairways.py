"""
ITA Airways connector — Italy's flag carrier (successor to Alitalia).

ITA Airways (IATA: AZ) — FCO/MXP hubs.
SkyTeam member. 70+ destinations across Europe, Americas, Africa, Asia.

Strategy:
  ita-airways.com — try lowfare/calendar API endpoints.
"""
...
_BASE = "https://www.ita-airways.com"
_HEADERS = {
    "User-Agent": (
...
class ITAAirwaysConnectorClient:
    """ITA Airways — lowfare calendar API."""
    ...
    async def search_flights(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        for endpoint in [
            ...
            try:
                ...
                logger.debug("ITA endpoint %s error: %s", endpoint, e)

        offers.sort(key=lambda o: o.price if o.price > 0 else float("inf"))
        elapsed = time.monotonic() - t0
        logger.info("ITA %s→%s: %d offers in %.1fs", req.origin, req.destination, len(offers), elapsed)

        sh = hashlib.md5(f"ita{req.origin}{req.destination}{req.date_from}".encode()).hexdigest()[:12]
    ...
    def _parse(self, data: dict, req: FlightSearchRequest, target_date: str) -> list[FlightOffer]:
        ...
        for fare in fares:
            ...
            seg = FlightSegment(
                airline="ITA Airways", flight_no="AZ", origin=req.origin,
                destination=req.destination, departure=dep_dt, arrival=dep_dt, duration_seconds=0,
            )
            route = FlightRoute(segments=[seg], total_duration_seconds=0, stopovers=0)
            oid = hashlib.md5(f"az_{req.origin}{req.destination}{target_date}{price}".encode()).hexdigest()[:12]
            offers.append(FlightOffer(
                id=f"az_{oid}", price=round(float(price), 2), currency=currency,
                price_formatted=f"{float(price):.2f} {currency}",
                outbound=route, inbound=None, airlines=["ITA Airways"], owner_airline="AZ",
                booking_url=f"https://www.ita-airways.com/en_us/booking?origin={req.origin}&destination={req.destination}&date={target_date}",
                is_locked=False, source="itaairways_direct", source_tier="free",
            ))
        return offers
