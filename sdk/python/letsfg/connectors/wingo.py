"""
Wingo connector — Copa Airlines subsidiary LCC.
...
Strategy:
  Wingo uses a Navitaire-based booking engine at booking.wingo.com.
  Calendar fares available via API endpoint.
"""
...
class WingoConnectorClient:
    """Wingo — Navitaire lowfare calendar API."""
    ...
    async def search_flights(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        for endpoint in [
            ...
            params = {
                ...
                "tripType": "1",
                "currencyCode": req.currency or "COP",
            }
            try:
    ...
    def _parse(self, data: dict, req: FlightSearchRequest, target_date: str) -> list[FlightOffer]:
        ...
        for fare in fares:
            ...
            currency = fare.get("currency") or fare.get("currencyCode") or "COP"
            if float(price) <= 0:
                continue
            dep_dt = datetime.combine(req.date_from, datetime.min.time().replace(hour=8))
            seg = FlightSegment(
                airline="Wingo", flight_no="P5", origin=req.origin,
                destination=req.destination, departure=dep_dt, arrival=dep_dt, duration_seconds=0,
            )
            route = FlightRoute(segments=[seg], total_duration_seconds=0, stopovers=0)
            oid = hashlib.md5(f"p5_{req.origin}{req.destination}{target_date}{price}".encode()).hexdigest()[:12]
            offers.append(FlightOffer(
                id=f"p5_{oid}", price=round(float(price), 2), currency=currency,
                price_formatted=f"{float(price):.2f} {currency}",
                outbound=route, inbound=None, airlines=["Wingo"], owner_airline="P5",
                booking_url=f"https://booking.wingo.com/search?origin={req.origin}&destination={req.destination}&date={target_date}&adults={req.adults or 1}",
                is_locked=False, source="wingo_direct", source_tier="free",
            ))
        return offers
    ...
    def _empty(req: FlightSearchRequest) -> FlightSearchResponse:
        return FlightSearchResponse(
            ...
            currency="COP", offers=[], total_results=0,
        )
