...
Strategy:
  Virgin Australia website at virginaustralia.com uses a low-fare calendar
  API. Try calendar/API endpoints first, fallback to generic pricing.
"""
...
_BASE = "https://www.virginaustralia.com"
_HEADERS = {
    "User-Agent": (
...
class VirginAustraliaConnectorClient:
    """Virgin Australia — lowfare calendar API."""
    ...
    async def search_flights(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        for endpoint in [
            ...
            try:
                ...
                logger.debug("VirginAustralia endpoint %s error: %s", endpoint, e)

        offers.sort(key=lambda o: o.price if o.price > 0 else float("inf"))
        elapsed = time.monotonic() - t0
        logger.info("VirginAustralia %s→%s: %d offers in %.1fs", req.origin, req.destination, len(offers), elapsed)
    ...
    def _parse(self, data: dict, req: FlightSearchRequest, target_date: str) -> list[FlightOffer]:
        offers = []
        fares = data.get("fares") or data.get("dates") or data.get("flights") or data.get("lowestFares") or []
        for fare in fares:
            ...
            seg = FlightSegment(
                airline="Virgin Australia", flight_no="VA", origin=req.origin,
                destination=req.destination, departure=dep_dt, arrival=dep_dt, duration_seconds=0,
            )
            route = FlightRoute(segments=[seg], total_duration_seconds=0, stopovers=0)
            oid = hashlib.md5(f"va_{req.origin}{req.destination}{target_date}{price}".encode()).hexdigest()[:12]
            offers.append(FlightOffer(
                id=f"va_{oid}", price=round(float(price), 2), currency=currency,
                price_formatted=f"{float(price):.2f} {currency}",
                outbound=route, inbound=None, airlines=["Virgin Australia"], owner_airline="VA",
                booking_url=f"https://www.virginaustralia.com/au/en/booking/flights/search/?origin={req.origin}&destination={req.destination}&date={target_date}&adults={req.adults or 1}",
                is_locked=False, source="virginaustralia_direct", source_tier="free",
            ))
        return offers
